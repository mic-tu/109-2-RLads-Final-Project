...
knitr::opts_chunk$set(echo = TRUE, out.width="100%", fig.width=12, fig.height=8)
```
...
### 教授有綽號是否代表與學生比較喜歡？
以下驗證有無綽號與私心推薦分數表現的關聯性，發現雖然有綽號的教授僅佔約2%，但觀察有綽號的教授在各分數階層的分布時可以發現，
大部分有綽號的教授在私心推薦分數的表現上都相當好，光是得分在4分以上者就佔了多數。推測可能因為教授較受學生歡迎才會獲得綽號。
...
high_review_text <- get_segged_text(high_review) %>% top_n(20)
low_review_text <- get_segged_text(low_review) %>% top_n(20)
